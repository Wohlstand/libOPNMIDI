...
int main(int argc, char *argv[])
{
    ...
    static struct opn2_MIDIPlayer    *midi_player = NULL; /* Instance of ADLMIDI player */
    ...
    /* Initialize ADLMIDI */
    midi_player = opn2_init(spec.freq);
    if (!midi_player)
    {
        fprintf(stderr, "Couldn't initialize OPNMIDI: %s\n", opn2_errorString());
        return 1;
    }
    ...
    /*opn2_setBank(midi_player, 68);*/

    /* Set using of custom bank (WOPL format) loaded from a file */
    opn2_openBankFile(midi_player, "/home/wohlstand/Yandex.Disk/Музыка/xg.wopn");

    /* Open the MIDI (or MUS, IMF or CMF) file to play */
    if (opn2_openFile(midi_player, music_path) < 0)
    {
        fprintf(stderr, "Couldn't open music file: %s\n", opn2_errorInfo(midi_player));
        SDL_CloseAudio();
        opn2_close(midi_player);
        return 1;
    }
...
void my_audio_callback(void *midi_player, Uint8 *stream, int len)
{
    struct OPN2_MIDIPlayer* p = (struct OPN2_MIDIPlayer*)midi_player;
    ...
    samples_count = opn2_play(p, samples_count, (short*)buffer);
