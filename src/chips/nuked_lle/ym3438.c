...
#include "verilator/segachips_ym3438.h"
...
#define FMOPNA_Clock FMOPNA_2612_Clock

const int ymf276 = 0;
const int ymf276mode = 0;
const int ym2608 = 1;
const int ver_ym3438 = 0;
fmopna_2612_t opna;
...
void OPN2_Reset(ym3438_t *chip, Bit32u rate, Bit32u clock)
{
    ...
    //if (chip_type == ym3438_type_ym2612)
    if (ym2608)
        chip->chip.flags = fm_flags_ym2612;

    if (ymf276)
    {
        chip->chip2.input.cs = 1;
        chip->chip2.input.rd = 0;
        chip->chip2.input.wr = 0;
        chip->chip2.input.address = 0;
        chip->chip2.input.data = 0;
        chip->chip2.flags = ymf276mode ? 0 : fmopn2_flags_ym3438;

        for (i = 0; i < 288; i++)
        {
            FMOPN2_Clock(&chip->chip2, 0);
            FMOPN2_Clock(&chip->chip2, 1);
        }

        chip->chip2.input.ic = 1;
        for (; i < 288 * 2; i++)
        {
            FMOPN2_Clock(&chip->chip2, 0);
            FMOPN2_Clock(&chip->chip2, 1);
        }
        chip->chip2.input.ic = 0;
    ...
    {
        SegaChips_YM3438_Init();
        segachips_ym3438_io.cs = 0;
        segachips_ym3438_io.rd = 1;
        segachips_ym3438_io.wr = 1;
        segachips_ym3438_io.a0 = 0;
        segachips_ym3438_io.a1 = 0;
        segachips_ym3438_io.data = 0;
        segachips_ym3438_io.ic = 1;
        segachips_ym3438_io.test = 1;

        for (i = 0; i < 200; i++)
        {
            SegaChips_YM3438_Eval(0);
            SegaChips_YM3438_Eval(1);
        }

        segachips_ym3438_io.ic = 0;
        for (i = 0; i < 200; i++)
        {
            SegaChips_YM3438_Eval(0);
            SegaChips_YM3438_Eval(1);
    ...
    {
        FM_SetCS(&chip->chip, 1);
        FM_SetRead(&chip->chip, 0);
        FM_SetWrite(&chip->chip, 0);
        FM_SetAddress(&chip->chip, 0);
        FM_SetData(&chip->chip, 0);

        for (i = 0; i < 288; i++)
        {
            FM_Clock(&chip->chip, 0);
            FM_Clock(&chip->chip, 1);
        }

        FM_SetIC(&chip->chip, 1);
        for (; i < 288 * 2; i++)
        {
            FM_Clock(&chip->chip, 0);
            FM_Clock(&chip->chip, 1);
        }
        FM_SetIC(&chip->chip, 0);
...
void OPN2_Clock(ym3438_t *chip, Bit32s *buffer)
{
    if (1)
    {
        ...
        if (ymf276)
        {
            ...
            {
                FMOPN2_Clock(&chip->chip2, 0);
                sum_l += chip->chip2.out_l;
                sum_r += chip->chip2.out_r;
                FMOPN2_Clock(&chip->chip2, 1);
                sum_l += chip->chip2.out_l;
                sum_r += chip->chip2.out_r;
                ...
                if (ymf276mode)
                {
                    if (!chip->o_bco && chip->chip2.o_bco)
                    {
                        if (chip->o_lro != chip->chip2.o_lro)
                        {
                            if (chip->o_lro)
                        ...
                        chip->dac_shifter = (chip->dac_shifter << 1) | chip->chip2.o_so;

                        chip->o_lro = chip->chip2.o_lro;
                    }
                    chip->o_bco = chip->chip2.o_bco;
                }
            }
            if (ymf276mode)
            {
                buffer[0] = chip->sample_l;
                buffer[1] = chip->sample_r;
            }
            else
            {
                sum_l /= 12;
        ...
        if (ym2608)
        {
            for (i = 0; i < 6; i++)
            {
                ...
                //printf("%i %i %i %i %i %i %i %i %i %i %i\n", chip->opna.clk1, chip->opna.clk2,
                //    chip->opna.bclk1, chip->opna.bclk2, chip->opna.cclk1, chip->opna.cclk2,
                //    chip->opna.dclk, chip->opna.rss_eclk1, chip->opna.rss_eclk2, chip->opna.rss_fclk1,
                //    chip->opna.rss_fclk2);
#ifndef FMOPNA_YM2612
                FMOPNA_Clock(&opna, 0);
                FMOPNA_Clock(&opna, 1);
                if (chip->o_s && !opna.o_s)
                {
                    if (chip->o_sh1 && !opna.o_sh1)
                    {
                        chip->dac_shifter2 = chip->dac_shifter ^ 0x8000;
                    }
                    if (chip->o_sh2 && !opna.o_sh2)
                    {
                        chip->dac_shifter2 = chip->dac_shifter ^ 0x8000;
                    }
                    if (chip->o_sh1)
                    ...
                    chip->dac_shifter = (chip->dac_shifter >> 1) | (opna.o_opo << 15);

                    chip->o_sh1 = opna.o_sh1;
                    chip->o_sh2 = opna.o_sh2;
                }
                chip->o_s = opna.o_s;
#endif
            }
            ...
            buffer[0] = chip->sample_l;
            buffer[1] = chip->sample_r;
            //buffer[0] = (short)opna.ac_fm_accm1[1];
            //buffer[1] = (short)opna.ac_fm_accm2[1];
#endif
            return;
        }
        if (ver_ym3438)
        {
            ...
            {
                SegaChips_YM3438_Eval(0);
                sum_l += segachips_ym3438_io.o_mol;
                sum_r += segachips_ym3438_io.o_mor;
                SegaChips_YM3438_Eval(1);
                sum_l += segachips_ym3438_io.o_mol;
                sum_r += segachips_ym3438_io.o_mor;
            }
            if (ymf276mode)
            {
                buffer[0] = segachips_ym3438_io.o_mol;
                buffer[1] = segachips_ym3438_io.o_mor;
            }
            else
            {
                sum_l /= 12;
        ...
        {
            FM_Clock(&chip->chip, 0);
            sum_l += chip->chip.out_l;
            sum_r += chip->chip.out_r;
            FM_Clock(&chip->chip, 1);
            sum_l += chip->chip.out_l;
            sum_r += chip->chip.out_r;
        }
        if (chip->chip.flags & fm_flags_ym2612)
        {
            sum_l /= 4;
            sum_r /= 4;
        }
...
void OPN2_Write(ym3438_t *chip, Bit32u port, Bit8u data)
{
    if (ymf276)
    {
        chip->chip2.input.address = port & 3;
        chip->chip2.input.data = data;
        chip->chip2.input.wr = 1;
        FMOPN2_Clock(&chip->chip2, 0);
        chip->chip2.input.wr = 0;
    }
    else if (ym2608)
    {
        opna.input.a0 = port & 1;
        opna.input.a1 = (port >> 1) & 1;
        opna.input.data = data;
        opna.input.wr = 0;
        FMOPNA_Clock(&opna, 1);
        opna.input.wr = 1;
    }
    ...
    {
        segachips_ym3438_io.a0 = port & 1;
        segachips_ym3438_io.a1 = (port >> 1) & 1;
        segachips_ym3438_io.data = data;
        segachips_ym3438_io.wr = 0;
        SegaChips_YM3438_Eval(1);
        segachips_ym3438_io.wr = 1;
    }
    else
    {
        FM_SetAddress(&chip->chip, port);
        FM_SetData(&chip->chip, data);
        FM_SetWrite
        (&chip->chip, 1);
        FM_SetWrite(&chip->chip, 0);
    }
...
void OPN2_GenerateResampled(ym3438_t *chip, Bit32s *buf)
{
    ...
    while (chip->samplecnt >= chip->rateratio)
    {
        ...
        if ((ymf276 || ver_ym3438) && ymf276mode)
        {
            chip->samples[0] /= 32 * 8 / 2;
            chip->samples[1] /= 32 * 8 / 2;
        }
#ifndef FMOPNA_YM2612
        if (ym2608)
        {
