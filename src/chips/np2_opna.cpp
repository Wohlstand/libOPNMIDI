...
template <>
void NP2OPNA<FM::OPNA>::writeReg(uint32_t port, uint16_t addr, uint8_t data)
{
    chip->SetReg((port << 8) | addr, data);
}

template <>
void NP2OPNA<FM::OPM>::writeReg(uint32_t port, uint16_t addr, uint8_t data)
{
    (void)port;
...
template <>
void NP2OPNA<FM::OPNA>::sendInitSequence()
{
    chip->SetReg(0x29, 0x9f);  // enable channels 4-6
}

template <>
void NP2OPNA<FM::OPM>::sendInitSequence()
{
}
...
template <>
const char *NP2OPNA<FM::OPM>::emulatorName()
{
    return "Neko Project II Kai OPM";  // git 2018-10-28 rev e1c0609
}

template class NP2OPNA<FM::OPNA>;
template class NP2OPNA<FM::OPM>;
