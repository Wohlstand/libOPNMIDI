...
//void OPN2::Poke(size_t card, uint32_t index, uint32_t value)
//{
...
//        {
//            uint8_t chx   = ch % 3;
//            uint8_t port = (chx <= 2) ? 0 : 1;
//            regBD[ch] = 0xC0;
//            PokeO(card, port, 0xB4 + ch, regBD[ch]);   //Panorame (toggle on both speakers)
//        }

        //for(unsigned a = 0; a < 18; ++a)
        //    Poke(card, 0xB0 + Channels[a], 0x00);
