...
static void initWorkarounds()
{
    ...
    if(s_winmm_dll)
    {
        ...
        if(!s_DriverCallback)
            MessageBoxW(NULL, L"Failed to get a workaround for DriverCallback", L"libOPNMIDI", MB_OK | MB_ICONEXCLAMATION);
    }
    else
    {
        MessageBoxW(NULL, L"Failed to open a library for a workaround for DriverCallback", L"libOPNMIDI", MB_OK | MB_ICONEXCLAMATION);
    }
}
...
EXTERN_C HRESULT modGetCaps(PVOID capsPtr, DWORD capsSize)
{
    ...
    CHAR synthName[] = "libOPNMIDI synth\0";
    WCHAR synthNameW[] = L"libOPNMIDI synth\0";
